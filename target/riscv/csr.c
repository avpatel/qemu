...
#define VSTOPI_NUM_SRCS 5
    ...
    hviid = get_field(env->hvicontrol, HVICONTROL_IID);
    hviprio = get_field(env->hvicontrol, HVICONTROL_IPRIO);

    if (gein) {
        if (gein <= env->geilen && (env->mie & env->mip & MIP_VSEIP)) {
    ...
    if (iid) {
        if (env->hvicontrol & HVICONTROL_IPRIOM) {
