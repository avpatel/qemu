...
static void riscv_imsic_update(RISCVIMSICState *imsic, uint32_t page)
{
    CPUState *cpu;
    CPURISCVState *env;
    uint32_t topei = riscv_imsic_topei(imsic, page);

    cpu = qemu_get_cpu(imsic->hartid);
    env = cpu ? cpu->env_ptr : NULL;
    if (!env) {
        return;
    }

    if (imsic->eidelivery[page] && topei) {
        riscv_cpu_update_mip(RISCV_CPU(cpu),
                             (imsic->mmode) ? MIP_MEIP : MIP_SEIP,
                             BOOL_TO_MASK(1));
    } else {
        riscv_cpu_update_mip(RISCV_CPU(cpu),
                             (imsic->mmode) ? MIP_MEIP : MIP_SEIP,
                                 ...
                                 target_ulong wr_mask)
{
    ...
    if (val) {
        *val = riscv_imsic_topei(imsic, page);
    }
                           ...
                           target_ulong new_val, target_ulong wr_mask)
{
    ...
    priv = IMSIC_REG_PRIV(reg);
    virt = IMSIC_REG_VIRT(reg);
    isel = IMSIC_REG_ISEL(reg);
    vgein = IMSIC_REG_VGEIN(reg);
...
static void riscv_imsic_realize(DeviceState *dev, Error **errp)
{
    ...
    if (env) {
        ...
        riscv_cpu_set_imsic_rmw_fn(env, (imsic->mmode) ? PRV_M : PRV_S,
